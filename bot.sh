...
if [[ -z "$1" && "$1"=="sort" ]]; then
	echo "execute retieve worker..."; nohup /root/retrieve.sh keep &
fi
if [ $countencoded != 0 ]; then
	echo "execute upload worker..."; nohup php /root/NodefilesUploader.php &
fi
...
if [[ $countsource != 0 ]]; then
	if [ $countqueue == 0 ]; then
		...
		if pidof -s ffmpeg > /dev/null; then
			...
			echo "execute encode..."; nohup /root/encode.sh &
		fi
	else
