...
if [[ -z "$1" && "$1"=="sort" ]]; then
	if ! pidof -s retrieve.sh > 0; then
		echo "execute retieve worker..."; nohup /root/app/retrieve.sh keep >/dev/null 2>&1 &
	fi
fi
if [ $countencoded != 0 ]; then
	if ! pidof -s php > 0; then
		echo "execute upload worker..."; nohup php /root/NodefilesUploader.php >> /root/log.txt &
	fi
fi
...
if [[ $countsource != 0 ]]; then
	...
	if [ $countqueue == 0 ]; then
		echo "execute rename..." >> /root/log.txt;
		mv ${SOURCE}/*.mkv ${QUEUE}; sleep 1
		if pidof -s encode.sh > 0 && pidof -s ffmpeg > 0; then
			if [[ ! $lastlog == "encode running..." ]]; then
				echo "encode running..." >> /root/log.txt
			fi
		else
